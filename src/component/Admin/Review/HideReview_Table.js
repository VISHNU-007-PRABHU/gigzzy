...
class HideReview_Table extends React.Component {
    state = {
        modalVisible: false,
        dataSource: [],
        update_data: {},
    ...
    componentDidMount() {
        this.fetch_category({
            ...
            data: { user_comments_status: 0, booking_status: [13, 14] }
        });
    }
    handleTableChange = async pagination => {
        ...
        await client.query({
            query: GET_REVIEW,
            variables: { limit: pager.pageSize, page: pager.current, data: { user_comments_status: 0, booking_status: [13, 14] } },
    ...
    onFilter_Ref = async (data) => {
        if (data.target.value) {
            this.fetch_category({
                ...
                after: this.state.pagination.cursor,
                data: {
                    user_comments_status: 0, booking_status: [13, 14],
    ...
    render() {
        const columns = [
            ...
            },
            {
                ...
                dataIndex: 'operation',
                render: (text, record) => {
                    return <span title="...." className="d-inline d-md-flex justify-content-around">
                        <Switch checkedChildren="View" unCheckedChildren="Hide" checked={record.user_comments_status} onChange={(value) => { this.update_comments_status(record._id, value) }} />
                    </span>
                }
            },
        ];
...
export default Form.create()(withRouter(HideReview_Table));