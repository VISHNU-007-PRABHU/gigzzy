...
class ProviderComponent extends React.Component {
    ...
    handleTableChange = async pagination => {
        ...
        await client.query({
            query: GET_USER,
            variables: { limit: pager.pageSize, page: pager.current, role: "2", proof_status: 1 },
    ...
    fetch_user = async (visible) => {
        ...
        await client.query({
            query: GET_USER,
            variables: { limit: this.state.pagination.pageSize, page: this.state.pagination.current, role: "2", proof_status: 1 },
            fetchPolicy: 'no-cache',
        }).then(result => {
            const pagination = { ...this.state.pagination };
            pagination.total = result.data.get_user.pageInfo.totalDocs;
    ...
    onFilter = async (value) => {
        ...
        var datas = { delete: 0, role: 2, proof_status: 1, $or: [{ 'first_name': { $regex: '.*' + value.target.value + '.*', $options: 'i' } }, { 'email': { $regex: '.*' + value.target.value + '.*', $options: 'i' } }, { 'phone_no': { $regex: '.*' + value.target.value + '.*', $options: 'i' } }] }
        await client.query({
                ...
                render: (text, record) =>
                    this.state.dataSource.length >= 1 ? (
                        <span title="...." className="d-inline d-md-flex justify-content-around">
                            ...
                            <RoleView permission="edit_provider">
                            ...
                            <RoleView permission="delete_provider">
...
export default withRouter(ProviderComponent);