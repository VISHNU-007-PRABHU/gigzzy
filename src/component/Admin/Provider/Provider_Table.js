...
class ProviderComponent extends React.Component {
    ...
    onFilter = async (value) => {
        ...
        var datas = { delete: 0, role: 2, proof_status: 1, $or: [{ 'name': { $regex: '.*' + value.target.value + '.*', $options: 'i' } }, { 'email': { $regex: '.*' + value.target.value + '.*', $options: 'i' } }, { 'phone_no': { $regex: '.*' + value.target.value + '.*', $options: 'i' } }] }
        await client.query({
