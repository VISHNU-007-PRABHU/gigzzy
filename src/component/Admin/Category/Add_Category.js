...
import { Layout, Icon, Form, Input,  Button, message, Typography, Row, Col, Select, Upload,Checkbox } from 'antd';
...
class Add_Category extends React.Component {
    ...
    fetch_find_subcategory=async()=>{
        await client.query({
            ...
            console.log(result);
            this.setState({update:1,update_data: result.data.category[0],certificate:result.data.category[0].Certificate,imageUrl:result.data.category[0].img_url,is_parent:result.data.category[0].is_parent});
        });
    }
     normFile = e => {
        if (Array.isArray(e)) {
            ...
            if (!err) {
                await client.mutate({
                    ...
                    variables: {
                        ...
                        category_name: values.category_name, description: values.description, file: this.state.file,is_parent:this.state.is_parent
                    },
        ...
        form.validateFields(async (err, values) => {
            if (!err) {
                await client.mutate({
                    ...
                    variables: {
                        base_price: values.base_price,hour_limit: values.hour_limit, hour_price: values.hour_price, service_fee: values.service_fee, certificates: values.certificates,
                        category_id: values.category_name, description: values.description, file: this.state.file,_id:this.props.match.params.id,is_parent:this.state.is_parent
                    },
                }).then((result, loading, error) => {
